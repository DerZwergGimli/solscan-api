...
mod test_block_last {
    ...
    async fn test_block_last_success() {
        let json_data_file = fs::read_to_string("./src/tests/test_endpoints/sample_data/sample_block_last.json").expect("Unable to read file");
        ...
        let result = solscan_api.get_block_last(Some(20)).await.unwrap();
